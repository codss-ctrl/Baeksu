...
def main():
    ...
    return render_template('main_view.html', select_mydday_title=b, select_mydday_date=d, mem_cnt=mem_cnt,
                           visit_cnt=visit_cnt, news_1=news_1, news_2=news_2, news_3=news_3, news_4=news_4,
                           news_5=news_5, enumerate=enumerate)
...
def logout():
    ...
    return render_template('main_view.html', select_mydday_title=select_mydday_title,
                           select_mydday_date=select_mydday_date, visit_cnt=visit_cnt, mem_cnt=mem_cnt, news_1=news_1,
                           news_2=news_2, news_3=news_3, news_4=news_4, news_5=news_5, enumerate=enumerate)
...
def after_mod_ajax():
    ...
    cnt = DaoAfter().update(after_seq, after_title, after_content, attach_file, attach_path, in_date, user_id, up_date,
                            user_id)
...
#########################################################
@app.route('/calendar')  # 일정캘린더
def calendar():
    flag_ses, user_id = getSession()
    if not flag_ses:
        return redirect("login")
    list = DaoCalendar().select_myplan(user_id)
...
def cal_addact():
    ...
    cnt = DaoCalendar().insert(plan_title, plan_content, plan_date, user_id)
    if cnt == 1:
        msg = 'ok'
    else:
        msg = 'ng'
...
def cal_modact():
    ...
    cnt = DaoCalendar().update(user_id, plan_seq, plan_title, plan_content)
    if cnt == 1:
        msg = 'ok'
    else:
        msg = 'ng'
...
def cal_mod():
    ...
    list = DaoCalendar().select_detail(plan_seq)
    plan_detail = list[0]
...
def cal_delact():
    ...
    cnt = DaoCalendar().delete(plan_seq)
    if cnt == 1:
        msg = 'ok'
    else:
        msg = 'ng'
...
def question_addact():
    ...
    cnt = MyDaoBoard().myinsert(board_title, board_content, attach_file, attach_path, '', mem_id, '', mem_id,
                                board_pass)
...
def question_modact():
    ...
    cnt = MyDaoBoard().myupdate(board_seq, board_title, board_content, attach_file, attach_path, "", user_id, "",
                                user_id)
